<p align="center">
  <img width="400" height="auto" alt="Sin título-1" src="https://github.com/user-attachments/assets/2ce9bf89-9ed8-4c6d-b958-462ebb34ccc8" />
</p>


<p align="center">
  <img width="auto" height="400" alt="Invasor-2" src="https://github.com/user-attachments/assets/89ff2a45-e3b7-4a5e-96c1-d894fc980aa8" />
</p>

<p align="center">
   <a href="https://dotnet.microsoft.com/">
    <img src="https://img.shields.io/badge/C%23-Ransomware-093fb9.svg">
  </a>
    <img src="https://img.shields.io/badge/System-Windows-093fb9.svg">
  </a>
    <img src="https://img.shields.io/badge/Private-%F0%9F%94%92-093fb9.svg">
  </a>
</p>

  <div id="header" align="center">
  <a href="https://github.com/DenverCoder1/readme-typing-svg"><img src="https://readme-typing-svg.herokuapp.com?size=14&width=220&color=093fb9&lines=Mira+primero,+luego+salta"></a>

  <p align="center">【 https://r3li4nt.github.io/Invasor 】</p>
    
</div>

<h1 align="center"></h1>

### ACERCA DE `Invasor Ransomware`:

Invasor es un ransomware desarrollado en C# puro para sistemas Microsoft Windows, cuenta con dos versiones especializadas:

**Offensive**: Implementa técnicas avanzadas utilizadas en amenazas reales, incluyendo mecanismos de cifrado, evasión de defensas, movimiento lateral y recolección de información. Está orientado a pruebas de resiliencia y evaluación de las capacidades de detección y respuesta dentro de infraestructuras corporativas.

**Simulator**: Versión enfocada en campañas de concientización y simulación de phishing, diseñada para evaluar la capacidad de reacción de los empleados ante incidentes de ransomware, sin comprometer datos ni sistemas reales de la organización.

<p align="center">
  <img width="1536" height="1024" alt="invasor-ransomware-1" src="https://github.com/user-attachments/assets/21568977-d32b-4d0e-9aa5-a602170e5c67" />
</p>

➤ <a href="https://github.com/R3LI4NT/Invasor/tree/main/versions/Offensive" target="__blank">Click para ver los módulos de Invasor Offensive</a>

➤ <a href="https://github.com/R3LI4NT/Invasor/tree/main/versions/Simulator" target="__blank">Click para ver los módulos de Invasor Simulator</a>

<h1 align="center"></h1>

¿Quieres evaluar las defensas de la infraestructura de tu empresa frente a ataques de ransomware con Invasor? ¿O prefieres poner a prueba la capacidad de respuesta de los empleados mediante simulacros de phishing con escenarios de Ransomware, sin comprometer la infraestructura?

Contacto:

<img src="https://img.shields.io/badge/r3li4nt.contact@keemail.me-D14836?style=for-the-badge&logo=gmail&logoColor=white" />

<h1 align="center"></h1>

> [!NOTE]
> Invasor esta programado en C#, sin utilizar código de otras muestras conocidas, kits de construcción o fragmentos reutilizados. Por lo tanto, su código fuente es único y privado. 

> Fecha de creación: Principio de Noviembre 2025
